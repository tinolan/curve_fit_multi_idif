...
def process_pet_slice(patient, slice_idx, slice_orientation="axial"):
    print(f"whole slice for aorta and ureter input {patient}, {slice_orientation}")
    ...
    save_path = os.path.join("/home/guests/valentin_langer/data/plots/organ_curve", "voxel", f"kidney_Vb_{slice_orientation}_{patient}_slice_{slice_idx}")
    ...
    aorta_idif, ureter_idif = KM_2TC.read_idif(time_stamp, t_file)

    shape = pet_slice_1.shape
    param_maps = {param: torch.full(shape, 0, dtype=torch.float32) for param in ["k1", "k2", "k3", "Vb", "alpha", "beta"]}  
    ...
    for coord in all_cords:
        ...
        if tuple(coord) in kidney_coords_list:
            real_kidney_tac_matrix.append(tac_interp)
            try:
    ...
    for param, data in param_maps.items():
        ...
        plt.title(f"{param} für Slice {slice_idx} (nur Niere)")
        plt.savefig(os.path.join(save_path, f"{patient}_slice_{slice_idx}_{param}_kidney.png"))
        plt.close()
    ...
    if(len(kidney_tac_matrix) > 0):
        ...
        plt.xlabel("Zeit (s)")
        plt.ylabel("kBq/ml")
        plt.title("Time-Activity-Curve für Niere")
        plt.legend()
        plt.xlim([0, 5])
...
def process_pet_slice_normal(patient, slice_idx, slice_orientation="axial"):
    print(f"whole slice kidney for aorta input {patient}, {slice_orientation}")
    ...
    label_map_path = glob.glob(root_data_path + f"/DynamicPET/*DynamicFDG_{patient}/NIFTY/Resampled/labels.nii.gz")[0]
    PET_list = natsorted(glob.glob(root_data_path + f"/DynamicPET/*DynamicFDG_{patient}/NIFTY/Resampled/PET_*.nii.gz"))
    save_path = os.path.join("/home/guests/valentin_langer/data/plots/organ_curve", "voxel", f"kidney_normal_{slice_orientation}_{patient}_slice_{slice_idx}")
    os.makedirs(save_path, exist_ok=True)
...
if __name__ == '__main__':
    ...
    process_pet_slice_normal(patient="06", slice_idx=161, slice_orientation="coronal")
