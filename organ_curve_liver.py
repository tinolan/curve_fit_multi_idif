...
def process_pet_liver(patient):
    print(f"Starting aorta+portal liver processing for patient {patient}")
    ...
    save_path = os.path.join("/home/guests/valentin_langer/data/plots/organ_curve", "image-derived", f"liver_Vb_{patient}")
    os.makedirs(save_path, exist_ok=True)
    
    label_map_ = sitk.GetArrayFromImage(sitk.ReadImage(label_map_path))
    liver_mask = (label_map_ == 3) 
    ...
    #liver_coords = liver_coords[:5] 
    
    for coord in liver_coords:
        z, x, y = coord
        ...
        try:
            p, _ = curve_fit(KM_2TC.PET_2TC_KM, t_file, tac_interp,
                             ...
                             bounds=([0.01, 0.01, 0.001, 0.001, 0, 0], [10, 10, 1, 0.05, 1, 1]),
                             diff_step=0.001)
...
def process_pet_liver_normal(patient):
    print(f"Starting normal liver processing for patient {patient}")
    time_stamp = torch.load(root_data_path + "/DynamicPET/time_stamp.pt")
    t_file = torch.load(root_data_path + "/DynamicPET/t.pt")
    t = torch.linspace(0, t_file[-1], 2000)
    ...
    label_map_path = glob.glob(root_data_path + f"/DynamicPET/*DynamicFDG_{patient}/NIFTY/Resampled/labels.nii.gz")[0]
    PET_list = natsorted(glob.glob(root_data_path + f"/DynamicPET/*DynamicFDG_{patient}/NIFTY/Resampled/PET_*.nii.gz"))
    save_path = os.path.join("/home/guests/valentin_langer/data/plots/organ_curve", "image-derived", f"liver_normal_{patient}")
    os.makedirs(save_path, exist_ok=True)
...
if __name__ == '__main__':
    process_pet_liver(patient="06")
    process_pet_liver_normal(patient="26")
