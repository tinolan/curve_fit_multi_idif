...
def process_pet_kidney(patient):
    print(f"Starting aorta+ureter kidney processing for patient {patient}")
    ...
    save_path = os.path.join("/home/guests/valentin_langer/data/plots/organ_curve", "image-derived", f"kidney_Vb_{patient}")
    os.makedirs(save_path, exist_ok=True)
    
    label_map_ = sitk.GetArrayFromImage(sitk.ReadImage(label_map_path))
    kidney_mask = np.isin(label_map_[:, :, :], [6, 7])
    ...
    #kidney_coords = kidney_coords[:5]
    
    for coord in kidney_coords:
        z, x, y = coord
...
def process_pet_kidney_normal(patient):
    print(f"Starting normal kidney processing for patient {patient}")
    time_stamp = torch.load(root_data_path + "/DynamicPET/time_stamp.pt")
    t_file = torch.load(root_data_path + "/DynamicPET/t.pt")
    t = torch.linspace(0, t_file[-1], 2000)
    ...
    label_map_path = glob.glob(root_data_path + f"/DynamicPET/*DynamicFDG_{patient}/NIFTY/Resampled/labels.nii.gz")[0]
    PET_list = natsorted(glob.glob(root_data_path + f"/DynamicPET/*DynamicFDG_{patient}/NIFTY/Resampled/PET_*.nii.gz"))
    save_path = os.path.join("/home/guests/valentin_langer/data/plots/organ_curve", "image-derived", f"kidney_normal_{patient}")
    os.makedirs(save_path, exist_ok=True)
...
if __name__ == '__main__':
    process_pet_kidney(patient="28")
    process_pet_kidney_normal(patient="28")
